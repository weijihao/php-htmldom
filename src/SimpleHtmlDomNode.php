...
/**
 * simple html dom node [简单的HTML DOM节点]
...
class SimpleHtmlDomNode
{
    ...
    // get dom node's inner html [得到的内部HTML DOM节点]
    ...
    // get dom node's outer text (with tag) [得到的DOM节点的外文本（标签）]
    ...
    // build node's text with tag [用标记构建节点文本]
...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    // [paperg函数将文本转换成一个字符集到另一个如果两组是不一样的。]
    ...
    */
    ...
    // camel naming conventions [骆驼命名规范]
